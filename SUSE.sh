...
calculate_file_hashes() {
    ...
    if command -v sha256sum >/dev/null 2>&1; then
        ...
        echo "No hash calculation tool available"
        return 1
    fi
    
    # Calculate hashes for all files using the selected tool
    if [ "$use_md5" = true ]; then
        echo "Using $hash_name algorithm (SHA256 not available)..."
    else
    ...
    if [ "$use_md5" = true ]; then
        echo "File hashes calculated using $hash_name (SHA256 not available)"
    else
